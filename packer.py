...
class PackerApp:
    ...
    def __init__(self, master, language):
        ...
        self.image_wizard = tk.PhotoImage(file = "library/packer-images/icon-circle.png") # Placeholder
        ...
        self.tab_paintjob_title.grid(row = 0, column = 0, columnspan = 3, pady = 20)
        ...
        self.tab_paintjob_image_wizard = ttk.Label(self.tab_paintjob, image = self.image_wizard)
        self.tab_paintjob_image_wizard.grid(row = 1, column = 2, padx = 10)
        ...
        self.tab_paintjob_option_wizard = ttk.Radiobutton(self.tab_paintjob, text = l("{PaintJobWizard}"), value = "wizard", variable = self.tab_paintjob_variable)
        self.tab_paintjob_option_wizard.grid(row = 2, column = 2, pady = 10)
        self.tab_paintjob_image_wizard.bind("<1>", lambda e: self.tab_paintjob_variable.set("wizard"))
        ...
        self.tab_paintjob_desc_wizard = ttk.Label(self.tab_paintjob, text = l("{PaintJobWizardDesc}\n"), wraplength = 200, justify = tk.CENTER)
        self.tab_paintjob_desc_wizard.grid(row = 3, column = 2, padx = 10, sticky = "n")
        ...
        self.tab_paintjob_button_next.grid(row = 4, column = 2, padx = 10, pady = 10, sticky = "e")
        ...
        # Wizard screen
        self.wizard_screen = ttk.Frame(self.container)
        self.wizard_notebook = ttk.Notebook(self.wizard_screen)
        self.wizard_notebook.pack(expand=True, fill="both", padx=10, pady=10)

        # Wizard - Step 1: Basic Info
        self.wizard_step1 = ttk.Frame(self.wizard_notebook)
        self.wizard_notebook.add(self.wizard_step1, text="ステップ1：基本情報")

        self.ws1_mod_name_label = ttk.Label(self.wizard_step1, text="MOD名:")
        self.ws1_mod_name_label.grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.ws1_mod_name_var = tk.StringVar()
        self.ws1_mod_name_entry = ttk.Entry(self.wizard_step1, textvariable=self.ws1_mod_name_var)
        self.ws1_mod_name_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

        self.ws1_author_name_label = ttk.Label(self.wizard_step1, text="作者名:")
        self.ws1_author_name_label.grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.ws1_author_name_var = tk.StringVar()
        self.ws1_author_name_entry = ttk.Entry(self.wizard_step1, textvariable=self.ws1_author_name_var)
        self.ws1_author_name_entry.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        self.ws1_next_button = ttk.Button(self.wizard_step1, text="次へ", command=lambda: self.wizard_notebook.select(1))
        self.ws1_next_button.grid(row=2, column=1, padx=5, pady=10, sticky="e")

        # Wizard - Step 2: Vehicle
        self.wizard_step2 = ttk.Frame(self.wizard_notebook)
        self.wizard_notebook.add(self.wizard_step2, text="ステップ2：車両選択")

        self.ws2_vehicle_label = ttk.Label(self.wizard_step2, text="ペイントを適用する車両を選択してください。")
        self.ws2_vehicle_label.pack(pady=10)

        self.ws2_vehicle_var = tk.StringVar()
        self.ws2_vehicle_dropdown = ttk.Combobox(self.wizard_step2, textvariable=self.ws2_vehicle_var, state="readonly")
        self.ws2_vehicle_dropdown.pack(pady=5)

        self.ws2_prev_button = ttk.Button(self.wizard_step2, text="戻る", command=lambda: self.wizard_notebook.select(0))
        self.ws2_prev_button.pack(side="left", padx=10, pady=10)
        self.ws2_next_button = ttk.Button(self.wizard_step2, text="次へ", command=lambda: self.wizard_notebook.select(2))
        self.ws2_next_button.pack(side="right", padx=10, pady=10)

        # Wizard - Step 3: Paint Job Details
    ...
    def switch_to_main_screen(self):
        self.setup_screen.grid_forget()
        if self.tab_paintjob_variable.get() == "wizard":
            self.wizard_screen.grid(row = 0, column = 0, padx = 10, pady = 10)
        else:
    ...
    def load_main_screen_variables(self): # Also grids and ungrids elements depending on said variables
        ...
        # Populate wizard vehicle dropdown
        wizard_vehicles = []
        for veh in self.truck_list + self.truck_mod_list + self.bus_mod_list + self.trailer_list + self.trailer_mod_list:
            wizard_vehicles.append(f"{veh.display_name} [{veh.display_author}]")
        self.ws2_vehicle_dropdown.config(values=wizard_vehicles)
